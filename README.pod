...
actually support this. cower will search based on the first span of 2 non-regex
characters it finds and filter using the regular expression provided,
afterwards. There's no guarantee that complex patterns will return accurate
results.
